...
{
	if (mistake >= NB_OF_MISTAKES)
		return NULL;
	return mistake_names[mistake];
}

enum mistake_name_e mistake_name_from_string(const char *name)
{
	for (int i = 0; i < NB_OF_MISTAKES; i++)
		if (!strcmp(name, mistake_names[i]))
			return i;
	return NB_OF_MISTAKES;
}

const style_error_t coding_style[] = {
	/*garbage files*/
	{
		"O1",
